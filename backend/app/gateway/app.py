...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
    ...
    # Quickstart onboarding API is mounted at /api/quickstart (fork-specific:
    # lets an operator add provider credentials from the web UI so a one-click
    # deployment needs no API keys in its environment).
