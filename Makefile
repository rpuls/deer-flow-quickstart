...
.PHONY: help config config-upgrade check check-agent-guidance install extension-install extension-list extension-enable extension-disable extension-remove setup doctor support-bundle detect-thread-boundaries detect-blocking-io dev dev-daemon start start-daemon nginx stop up down clean docker-init docker-start docker-stop docker-logs docker-logs-frontend docker-logs-gateway docker-logs-redis setup-sandbox quickstart-up quickstart-down quickstart-logs quickstart-rebuild quickstart-test quickstart-e2e
...
help:
	...
	@echo ""
	@echo "Quickstart (one-click deployment shape: gateway + web + Postgres, no API keys):"
	@echo "  make quickstart-up      - Build and start the quickstart stack on http://localhost:3000"
	@echo "  make quickstart-down    - Stop it and delete its volumes"
	@echo "  make quickstart-logs    - Follow its logs"
	@echo "  make quickstart-rebuild - Rebuild the images and restart"
	@echo "  make quickstart-test    - Backend unit tests for the onboarding layer"
	@echo "  make quickstart-e2e     - Full-stack onboarding e2e (needs quickstart-up)"
	@echo ""
...
# ==========================================
# Quickstart: the one-click deployment shape
# ==========================================
# Same images and entrypoints the cloud template uses, with a local Postgres.
# Deliberately credential-free: start it, create the admin account, then add a
# provider under Settings -> Providers.

QUICKSTART_COMPOSE = docker compose -f docker/docker-compose.quickstart.yaml

quickstart-up:
	$(QUICKSTART_COMPOSE) up -d --build
	@echo "DeerFlow quickstart is starting on http://localhost:3000"

quickstart-down:
	$(QUICKSTART_COMPOSE) down -v

quickstart-logs:
	$(QUICKSTART_COMPOSE) logs -f

quickstart-rebuild:
	$(QUICKSTART_COMPOSE) build --no-cache
	$(QUICKSTART_COMPOSE) up -d --force-recreate

quickstart-test:
	cd backend && uv run pytest tests/test_quickstart.py

# Drives a real Gateway, so the stack must already be up (make quickstart-up).
quickstart-e2e:
	cd frontend && $(FRONTEND_PNPM) exec playwright test -c playwright.quickstart.config.ts
